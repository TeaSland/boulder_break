...
void InitialiseStory(char* paragraph)
{
	...
	// Making up the story
	char summary[] = "An explorer entered a maze cave. To escape he/she must:\n"
	...
	// Assigning the story to the output variable.
	strcpy(paragraph, summary);
}
...
int IsExitAccessible(int cave[CAVE_SIZE][CAVE_SIZE])
{
	...
	for (int i = 0; i < CAVE_SIZE; i++) {
		...
		for (int j = 0; j < CAVE_SIZE; j++) {
			...
			// Finds the exit and checks if it's on the border
			if (cave[i][j] == 'E' && (i == 0 || j == 0 || i == CAVE_SIZE - 1 || j == CAVE_SIZE)) {
